// SVG path data constants for visibility toggle
const EYE_OPEN_PATH =
...
async function initSettingsUI() {
  ...
  // Save button handler
  saveButton.addEventListener("click", () => saveToken(tokenInput.value));

  // Clear cache handler
  cacheClearButton.addEventListener("click", clearCache);
}
...
async function saveToken(token) {
  ...
  try {
    await browser.storage.local.set({ gitlabToken: trimmed });
